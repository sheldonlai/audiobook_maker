...
from text_utils.format_chapter_title import format_chapter_title 
...
class AudiobookModel:
    ...
    def create_audio_text_map(self, directory_path, sentences_list):
        ...
        for idx, sentence in enumerate(sentences_list):
            generated = False
            audio_path = ""
            new_text_audio_map[str(idx)] = self.default_text_audio_map_format(
                sentence=sentence, audio_path=audio_path, generated=generated)
        
        # print(new_text_audio_map)
        # if len(new_text_audio_map) > 0:
        #     print(f'Processing chapter title: {new_text_audio_map["0"]}')
        #     new_text_audio_map[0] = format_chapter_title(new_text_audio_map["0"])
        #     print(f'Processed chapter title: {new_text_audio_map["0"]}')
    ...
    def delete_sentences(self, rows_list):
        sorted_items = sorted(self.text_audio_map.items(),
                              key=lambda x: int(x[0]))
        filtered_items = [(k, v)
                          for k, v in sorted_items if int(k) not in rows_list]
        adjusted_items = [(str(i), v)
                          for i, (_, v) in enumerate(filtered_items, start=0)]
    ...
    def export_audiobook(self, directory_path, pause_duration):
        ...
        if not os.path.exists(audio_map_path):
            raise FileNotFoundError(
                "The selected directory is not a valid Audiobook Directory.")
        with open(audio_map_path, 'r', encoding='utf-8') as file:
            text_audio_map = json.load(file)
        sorted_audio_paths = [text_audio_map[key]['audio_path']
                              for key in sorted(text_audio_map, key=lambda k: int(k))]
        ...
        def probe_audio_properties(file_path):
            ...
            if len(lines) != 3:
                raise ValueError(
                    "ffprobe did not return sample_rate, channels, and bits_per_sample")
        ...
        if pause_duration > 0:
            ...
            if not sorted_audio_paths:
                raise ValueError(
                    "No audio files found to determine silence properties.")
            first_audio_path = sorted_audio_paths[0]
            sample_rate, channels, bits_per_sample = probe_audio_properties(
                first_audio_path)
            silence = AudioSegment.silent(
                duration=pause_length, frame_rate=sample_rate)
        ...
        while True:
            ...
            if not os.path.exists(new_audiobook_path):
                break
            idx += 1
        self.execute_subprocess([AudioSegment.silent(
            0).ffmpeg, '-f', 'concat', '-safe', '0', '-i', file_list_path, new_audiobook_path])
    ...
    def all_generated(self):
        ...
        generated_count = sum(
            1 for entry in self.text_audio_map.values() if entry['generated'])
    ...
    def generate_audio_for_sentence_threaded(self, directory_path, is_continue, is_regen_only, report_progress_callback, sentence_generated_callback, should_stop_callback=None):
        ...
        if is_regen_only:
            total_sentences = sum(
                1 for entry in self.text_audio_map.values() if entry['regen'])
        ...
        for speaker_id, entries in sentences_by_speaker.items():
            ...
            self.load_selected_tts_engine(
                tts_engine_name, speaker_id, **speaker_settings)
            use_s2s = speaker_settings.get('use_s2s', True)
            if use_s2s:
                ...
                if s2s_engine_name:
                    ...
                    s2s_validated = self.load_selected_s2s_engine(
                        s2s_engine_name, speaker_id, **s2s_parameters)
                else:
                    s2s_validated = False
            else:
                s2s_validated = False
            for idx, entry in entries:
                ...
                audio_path = self.generate_audio_proxy(
                    sentence, speaker_settings, s2s_validated)
                if audio_path:
                    new_audio_path = os.path.join(
                        directory_path, f"audio_{idx}.wav")
                ...
                progress_percentage = int(
                    (generated_count / total_sentences) * 100)
    ...
    def generate_audio_proxy(self, sentence, voice_parameters, s2s_validated):
        ...
        success = tts_engines.generate_audio(
            self.tts_engine, sentence, voice_parameters, tts_engine_name, audio_path)
        if not success:
            return None
        if s2s_validated:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.wav') as tmp_s2s_file:
                s2s_audio_path = tmp_s2s_file.name
            s2s_engines.process_audio(self.s2s_engine, s2s_engine_name=s2s_engine_name,
                                      input_audio_path=audio_path, output_audio_path=s2s_audio_path, parameters=voice_parameters)
    ...
    def get_map_keys_and_values(self, idx_str):
        ...
        return sentence, row_position, speaker_id, speaker_name, regen_state, generated

    def get_s2s_engines(self):
        s2s_config = self.load_config(
            os.path.join('configs', 's2s_config.json'))
    ...
    def get_tts_engines(self):
        tts_config = self.load_config(
            os.path.join('configs', 'tts_config.json'))
    ...
    def get_voice_indexes(self):
        if os.path.exists(self.index_folder_path) and os.path.isdir(self.index_folder_path):
            voice_index_files = [file for file in os.listdir(
                self.index_folder_path) if file.endswith(".index")]
    ...
    def get_voice_models(self):
        if os.path.exists(self.voice_folder_path) and os.path.isdir(self.voice_folder_path):
            voice_model_files = [file for file in os.listdir(
                self.voice_folder_path) if file.endswith(".pth")]
    ...
    def load_generation_settings(self, directory_path):
        generation_settings_path = os.path.join(
            directory_path, "generation_settings.json")
    ...
    def load_selected_s2s_engine(self, chosen_s2s_engine, speaker_id, **kwargs):
        if (self.current_s2s_engine_name == chosen_s2s_engine and
            ...
            try:
                self.s2s_engine = s2s_engines.load_s2s_engine(
                    chosen_s2s_engine, **kwargs)
    ...
    def load_selected_tts_engine(self, chosen_tts_engine, speaker_id, **kwargs):
        if (self.current_tts_engine_name == chosen_tts_engine and
            ...
            self.tts_engine = tts_engines.load_tts_engine(
                chosen_tts_engine, **kwargs)
    ...
    def load_sentences(self, file_path):
        ...
        if len(filtered_sentences) > 0:
            print(f'Processing chapter title: {filtered_sentences[0]}')
            filtered_sentences[0] = format_chapter_title(filtered_sentences[0])
            print(f'Processed chapter title: {filtered_sentences[0]}')
    ...
    def paragraph_to_sentence(self, paragraph) -> list:
        ...
        sentence_list = [s.strip() for s in paragraph.split(
    ...
    def reset(self, skip_reset_voice_setting=False):
        ...
        if not skip_reset_voice_setting:
            self.speakers = {
                "1": {'name': 'Narrator', 'color': '#FFFFFF', 'settings': {}}
            }
            self.current_tts_engine_name = None
    ...
    def save_generation_settings(self, directory_path, speakers=None):
        ...
        for speaker in speakers.values():
            ...
            if isinstance(color, QColor):
                ...
                raise TypeError(
                    f"Unsupported color type: {type(color)} for speaker {speaker}, value: {color}")
    ...
    def save_text_audio_map(self, directory_path):
        ...
        with open(map_file_path, 'w', encoding="utf-8") as map_file:
            json.dump(self.text_audio_map, map_file,
    ...
    def update_audiobook(self, directory_path, new_sentences_list):
        ...
        for new_idx, sentence in enumerate(new_sentences_list):
            ...
            if sentence in reverse_map and len(reverse_map[sentence]) >= count:
                ...
                old_audio_full_path = os.path.join(
                    directory_path, old_audio_filename)
                deleted_indices.discard(old_idx)
                new_audio_filename = f"audio_{new_idx}.wav"
                new_audio_full_path = os.path.join(
                    directory_path, new_audio_filename)
                ...
                generated = False
                new_audio_filename = ""
                new_text_audio_map[str(new_idx)] = self.default_text_audio_map_format(
                    sentence=sentence, audio_path=new_audio_filename, generated=generated)
        for old_idx in deleted_indices:
            ...
            if old_audio_path:
                ...
                if os.path.exists(old_audio_full_path):
                    ...
                    print(
                        f"Audio file to delete not found: {old_audio_full_path}")
        temp_files = {}
    ...
    def update_text_audio_map(self, sentences_list):
        new_text_audio_map = {}
        sentence_to_existing_idx = {
            item['sentence']: idx for idx, item in self.text_audio_map.items()}
        for idx, sentence in enumerate(sentences_list):
