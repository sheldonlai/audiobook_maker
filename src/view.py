...
from PySide6.QtWidgets import (
    ...
    QHeaderView, QProgressBar, QGridLayout, QHBoxLayout, QTableWidget,
    # is written in Python and it
    # seems to be related to PyQt,
    # a set of Python bindings for
...
class SpeakerManagementDialog(QDialog):
    ...
    def add_speaker(self):
        new_speaker_id = max(self.speakers.keys()) + \
            1 if self.speakers else 2  # Start from 2
        speaker_name, ok = QInputDialog.getText(
            self, "Add Speaker", "Enter Speaker Name:")
    ...
    def edit_speaker(self, item):
        ...
        if speaker:
            # Let user edit name
            speaker_name, ok = QInputDialog.getText(
                self, "Edit Speaker", "Enter Speaker Name:", text=speaker['name'])
            if ok and speaker_name:
                speaker['name'] = speaker_name
                # Let user select a color
                color = QColorDialog.getColor(
                    initial=speaker.get('color', Qt.black))
                if color.isValid():
                    speaker['color'] = color
                self.populate_speaker_list()
    ...
    def delete_speaker(self):
        ...
        if not selected_items:
            QMessageBox.warning(self, "Delete Speaker",
        ...
        for item in selected_items:
            ...
            if speaker_id == 1:
                QMessageBox.warning(self, "Delete Speaker",
                                    "Cannot delete the default speaker.")
                continue
            # Remove the speaker
...
class WordReplacerView(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.extra_replacement_checkbox = QCheckBox(
            "Do Extras i.e abbreviations, <>, etc.", self)
        ...
        self.word_widget.setHorizontalHeaderLabels(
        ...
        self.word_widget.setSizePolicy(
    ...
    def load_stylesheet(self, font_size="14pt"):
        ...
        # Replace font-size
        modified_stylesheet = stylesheet.replace(
            "font-size: 14pt;", f"font-size: {font_size};")
        return modified_stylesheet

    def load_word_list(self):
        # Load list of words to be replaced by something else
        options = QFileDialog.Options()
        self.word_path, _ = QFileDialog.getOpenFileName(
            self, "Select Wordlist File", "", "JSON Files (*.json);;All Files (*)", options=options)
        self.list_name_input.setText(os.path.basename(self.word_path))
        ...
        if not os.path.exists(self.word_path):
            QMessageBox.warning(
                self, "Error", "The selected directory does not contain a wordlist.")
            return
        try:
    ...
    def prompt_question(self, title, question, buttons=QMessageBox.Yes | QMessageBox.No, default_button=QMessageBox.No):
        reply = QMessageBox.question(
            self, title, question, buttons, default_button)
    ...
    def test_repl(self):
        ...
        if selected_row == -1 or selected_col == -1:  # No row is selected
            QMessageBox.warning(
                self, "Error", 'Choose a row and column to test')
            return
    ...
    def update_speaker_selection_combo(self, list_of_speakers):
        ...
        for speaker_id, speaker in list_of_speakers:
            self.speakers_combo.addItem(
                f"{speaker['name']}", userData=speaker_id)
...
class AudiobookMakerView(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.tts_engine_combo.currentTextChanged.connect(
        ...
        self.s2s_engine_combo.currentTextChanged.connect(
            self.on_s2s_engine_changed)
        ...
        pause = 0
        # 0 is the initial value of the slider
        ...
        self.export_pause_slider.valueChanged.connect(
            self.on_export_pause_slider_changed)
        ...
        self.start_generation_button = QPushButton(
            "Start Audiobook Generation", self)
        self.start_generation_button.clicked.connect(
            self.on_generate_button_clicked)
        self.stop_generation_button = QPushButton("Stop", self)
        self.stop_generation_button.clicked.connect(
        ...
        self.regenerate_button = QPushButton(
            "Regenerate Chosen Sentence", self)
        self.regenerate_button.clicked.connect(
            self.on_regenerate_button_clicked)

        self.regenerate_bulk_button = QPushButton("Regenerate in Bulk", self)
        self.regenerate_bulk_button.clicked.connect(
            self.on_regenerate_bulk_button_clicked)
        self.clear_regen_button = QPushButton("Clear Regen Checkboxes", self)
        self.clear_regen_button.clicked.connect(
            self.on_clear_regen_button_clicked)
        ...
        self.continue_audiobook_button = QPushButton(
            "Continue Audiobook Generation", self)
        self.continue_audiobook_button.clicked.connect(
            self.on_continue_button_clicked)
        ...
        self.tts_options_scroll_area.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.tts_options_widget.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        self.s2s_options_scroll_area.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        self.search_across_sentences = QCheckBox(
            "Search across sentences limits")
        ...
        # necessary so that the field has a decent size
        self.go_to_sentence_input.setMaximum(2**31 - 1)
        ...
        self.speaker_selection_layout.addWidget(
            self.speaker_selection_combo, 1)
        self.speaker_selection_combo.currentIndexChanged.connect(
            self.on_current_speaker_changed)
        ...
        self.load_audiobook_action.triggered.connect(
            self.on_load_existing_audiobook_triggered)
        self.update_audiobook_action = QAction(
            "Update Audiobook Sentences", self)
        self.update_audiobook_action.triggered.connect(
            self.on_update_audiobook_triggered)
        self.export_audiobook_action = QAction("Export Audiobook", self)
        self.export_audiobook_action.triggered.connect(
            self.on_export_audiobook_triggered)
        ...
        self.set_background_action.triggered.connect(
            self.on_set_background_image_triggered)
        self.set_background_clear_action = QAction(
            "Clear Background Image", self)
        self.set_background_clear_action.triggered.connect(
            self.on_set_background_clear_image_triggered)
        ...
        self.toggle_delete_action.triggered.connect(
            self.on_toggle_delete_action_triggered)
        ...
        self.word_replacer_action.toggled.connect(
            self.toggle_word_replacer_window)
    ...
    def add_table_item(self, row, text, speaker_name, regen_state=False, delete_state=False, is_generated=False):
        ...
        regen_check_box.stateChanged.connect(
            lambda state, row=row: self.regen_checkbox_toggled.emit(
                row, bool(state))
        )
    ...
    def assign_speaker_to_selected(self, speaker_id, speaker_name, text_to_audio_map):
        selected_rows = self.tableWidget.selectionModel().selectedRows()
        for index in selected_rows:
            row = index.row()
            is_generated = text_to_audio_map[str(row)]["generated"]
            self.set_row_speaker(row, speaker_id, speaker_name, is_generated)
            self.sentence_speaker_changed.emit(row, speaker_id)

    def browse_file(self, widget, param):
        file_path = self.get_open_file_name(
            "Select File", "", param.get('file_filter', 'All Files (*)'))
        if file_path:
    ...
    def create_widget_for_parameter(self, param):
        ...
        if param_type == 'text':
            ...
            widget.textChanged.connect(
                lambda text, attr=attribute: self.on_parameter_changed(attr, text))
            ...
            browse_button.clicked.connect(
                lambda _, w=widget, p=param: self.browse_file(w, p))
            layout.addWidget(widget)
            ...
            widget.valueChanged.connect(
                lambda value, attr=attribute: self.on_parameter_changed(attr, value))
            layout.addWidget(widget)
        elif param_type == 'checkbox':
            widget = QCheckBox()
            widget.stateChanged.connect(
                lambda state, attr=attribute: self.on_parameter_changed(attr, bool(state)))
            layout.addWidget(widget)
            default_value = param.get('default', False)
            ...
            def get_items_func(): return self.get_combobox_items(param)
            ...
            if relies_on:
                ...
                if relies_on_widget:
                    ...
                    relies_on_widget.currentTextChanged.connect(
                        update_items_based_on_dependency)
                    # Initially populate the combobox
                    items = get_items_func()
                    widget.addItems(items)
                else:
                    self.show_message(
                        "Error", f"Parameter '{attribute}' relies on unknown parameter '{relies_on}'", QMessageBox.Warning)
            else:
                if function_name == 'get_combobox_items':
                    ...
                    self.show_message(
                        "Error", f"Unknown function {function_name} for combobox parameter", QMessageBox.Warning)
            ...
            self.show_message(
                "Error", f"Unknown parameter type: {param_type}", QMessageBox.Warning)
            return None
    ...
    def get_combobox_items(self, param):
        ...
        none_option_label = param.get('none_option_label', 'Default')
        ...
        if relies_on:
            ...
            if relies_on_widget:
                ...
                self.show_message(
                    "Error", f"Parameter relies on unknown parameter '{relies_on}'", QMessageBox.Warning)
        ...
        if not os.path.exists(folder_path):
            self.show_message(
                "Error", f"Folder {folder_path} does not exist.", QMessageBox.Warning)
        ...
        if look_for == 'folders':
            ...
            try:
                for entry in os.scandir(folder_path):
                    if entry.is_dir() and any(fnmatch.fnmatch(entry.name, pattern) for pattern in patterns):
                        items.append(entry.name)
            except Exception as e:
                self.show_message(
                    "Error", f"Error reading directory {folder_path}: {e}", QMessageBox.Warning)
                return items
        elif look_for == 'files':
            patterns = file_filter.split(';')
            try:
            ...
            self.show_message(
                "Error", f"Invalid look_for value: {look_for}", QMessageBox.Warning)
            return items
    ...
    def get_s2s_engine_parameters(self):
        ...
        engine_config = next((engine for engine in self.s2s_config.get(
            's2s_engines', []) if engine['name'] == engine_name), None)
    ...
    def get_tts_engine_parameters(self):
        ...
        engine_config = next((engine for engine in self.tts_config.get(
            'tts_engines', []) if engine['name'] == engine_name), None)
        if not engine_config:
            return {}
    ...
    def load_speaker_settings(self, speaker_id):
        ...
        if index_tts >= 0:
            ...
            self.tts_engine_combo.setCurrentIndex(
                0)  # Default to first TTS engine
        ...
        # Update s2s engine
        s2s_engine = settings.get(
            's2s_engine', self.s2s_engine_combo.currentText())
        ...
        if index_s2s >= 0:
            ...
            self.s2s_engine_combo.setCurrentIndex(
                0)  # Default to first s2s engine
        ...
        use_s2s = settings.get('use_s2s', True)
    ...
    def load_tts_config(self, config_path):
        if not os.path.exists(config_path):
            self.show_message(
                "Error", f"Configuration file {config_path} not found.", QMessageBox.Critical)
            return {}
    ...
    def load_s2s_config(self, config_path):
        if not os.path.exists(config_path):
            self.show_message(
                "Error", f"s2s configuration file {config_path} not found.", QMessageBox.Critical)
            return {}
        with open(config_path, 'r') as f:
            return json.load(f)
    ...
    def on_go_to_sentence(self):
        self.select_table_row(
            min(self.tableWidget.rowCount(), self.go_to_sentence_input.value()) - 1)
    ...
    def on_next_search(self):
        self.search_sentences_requested.emit(self.get_search_start(
        ), True, self.search_input.text(), self.search_across_sentences.isChecked())
    ...
    def on_previous_search(self):
        self.search_sentences_requested.emit(self.get_search_start(
        ), False, self.search_input.text(), self.search_across_sentences.isChecked())
    ...
    def on_tts_engine_changed(self, engine_name):
        ...
        # Update the TTS options in the view
        self.update_tts_options(engine_name)
    ...
    def open_word_replacer_window(self, parent=None):
        ...
        self.word_replacer_window.window_closed.connect(
            self.on_word_replacer_closed)

    def populate_s2s_engines(self):
        s2s_config = self.load_s2s_config('configs/s2s_config.json')
        engines = [engine['name']
                   for engine in s2s_config.get('s2s_engines', [])]
    ...
    def reset(self, skip_reset_speech_options=False):
        self.clear_table()
        ...
        if not skip_reset_speech_options:
            self.tts_engine_combo.setCurrentIndex(0)
    ...
    def reset_settings_to_default(self):
        ...
        if default_speaker_id in self.speakers:
            ...
            self.show_message(
                "Error", "Default speaker not found.", QMessageBox.Critical)
    ...
    def set_row_speaker(self, row, speaker_id, speaker_name, is_generated=False):
        self.set_row_speaker_color(row, speaker_id, is_generated)
        speaker_item = QTableWidgetItem(speaker_name)
        speaker_item.setFlags(speaker_item.flags() & ~Qt.ItemIsEditable)
        self.tableWidget.setItem(row, 1, speaker_item)

    def set_row_speaker_color(self, row, speaker_id, is_generated=False):
        self.tableWidget.blockSignals(True)
        ...
        # Differentiate if something is generated with a light tone.
        if is_generated:
            if color == Qt.black:
                # black does not work well with the lighter function, explicitly setting the values
    ...
    def set_s2s_parameters(self, settings):
        ...
        engine_config = next(
            (engine for engine in self.s2s_config.get(
                's2s_engines', []) if engine['name'] == s2s_engine),
            None
        )
        if engine_config:
            for param in engine_config.get('parameters', []):
    ...
    def set_tts_parameters(self, settings):
        ...
        engine_config = next(
            (engine for engine in self.tts_config.get(
                'tts_engines', []) if engine['name'] == tts_engine),
            None
        )
        if engine_config:
            for param in engine_config.get('parameters', []):
                ...
                if widget:
                    ...
                    if value is not None:
                        ...
                        if param['type'] == 'combobox':
                            # Default to first item when value is None
                            widget.setCurrentIndex(0)
    ...
    def update_background(self):
        ...
        if hasattr(self, 'background_pixmap'):
            scaled_pixmap = self.background_pixmap.scaled(
                self.background_label.size(), Qt.KeepAspectRatioByExpanding)
    ...
    def update_generation_settings(self, settings):
        ...
        if index >= 0:
            ...
            self.speaker_selection_combo.setCurrentIndex(
                0)  # Default to first speaker

        # Load settings for current speaker
        self.load_speaker_settings(default_speaker_id)

    def update_speaker_selection_combo(self):
        self.speaker_selection_combo.blockSignals(
            True)  # Prevent signal during update
        self.speaker_selection_combo.clear()
        for speaker_id, speaker in self.speakers.items():
            self.speaker_selection_combo.addItem(
                f"{speaker['name']}", userData=speaker_id)
    ...
    def update_s2s_options(self, engine_name):
        ...
        engine_config = next(
            (engine for engine in self.s2s_config.get(
                's2s_engines', []) if engine['name'] == engine_name),
            None
        )

        if not engine_config:
            self.show_message(
                "Error", f"No configuration found for s2s engine: {engine_name}", QMessageBox.Critical)
            self.s2s_options_widget.setVisible(False)
            return

        # Create a label for the s2s engine
        engine_label = QLabel(engine_config.get(
            'label', f"{engine_name} Settings"))
    ...
    def update_tts_options(self, engine_name):
        ...
        if not engine_config:
            self.show_message(
                "Error", f"No configuration found for TTS engine: {engine_name}", QMessageBox.Critical)
            self.tts_options_widget.setVisible(False)
            return
