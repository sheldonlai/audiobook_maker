...
from src.widget.text_file_queue import TextFileQueue  # Assuming your class is in text_file_queue.py
...
class TestTextFileQueue(unittest.TestCase):
    ...
    def test_updateView_valid_input(self):
        file_queue = ["/path/to/file1.txt", "/path/to/file2.txt", "/path/to/file3.txt"]
        ...
        self.assertEqual(self.text_file_queue.queue_widget.rowCount(), len(file_queue))
        ...
        active_item = self.text_file_queue.queue_widget.item(1, 0)  # Index 1 for file2.txt
        self.assertEqual(active_item.background(), Qt.green) #check background color
    ...
    def test_get_audiobook_subdirectory(self):
        file_queue = ["/path/to/audiobook/file1.txt", "/path/to/audiobook/file2.txt", "/path/to/audiobook/file3.txt"]
        active_file_path = "/path/to/audiobook/file2.txt"
        self.text_file_queue.updateView(active_file_path, file_queue)

        subdir = self.text_file_queue.get_audiobook_subdirectory()
        self.assertEqual(subdir, "file2")  # Expecting the filename without extension
    ...
    def test_extract_relative_paths_for_display(self):
        paths = ["/path/to/audiobook/file1.txt", "/path/to/audiobook/file2.txt", "/path/to/audiobook/file3.txt"]
        expected_relative_paths = ["file1.txt", "file2.txt", "file3.txt"]
        relative_paths = self.text_file_queue._TextFileQueue__extract_relative_paths_for_display(paths) #access protected function
    ...
    def test_extract_common_prefix(self):
         ...
         prefix = self.text_file_queue._TextFileQueue__extract_common_prefix(paths) #access protected function
    ...
    def test_strip_extension(self):
        ...
        stripped_filename = self.text_file_queue._TextFileQueue__strip_extension(filename) #access protected function
